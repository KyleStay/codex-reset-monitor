...
const noServerTimeZone = () => null;

export function ObservationForm() {
  const [status, setStatus] = useState<{ kind: "idle" | "busy" | "success" | "error"; message: string }>({ kind: "idle", message: "" });
  const timeZone = useSyncExternalStore(subscribeToBrowserTimeZone, browserTimeZone, noServerTimeZone);

  function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    try {
      if (!timeZone) throw new Error("Your browser did not report a valid time zone.");
      const limitReached = localDateTimeToUtc(String(form.get("limitReachedAtUtc")));
      const resetAt = localDateTimeToUtc(String(form.get("observedResetAtUtc")));
      if (Date.parse(resetAt) <= Date.parse(limitReached)) throw new Error("Access-returned time must be later than the limit-reached time.");
  ...
  return (
    <form className="form-card" onSubmit={submit}>
      <div className="form-grid">
        ...
        <div className="field"><label htmlFor="timezone">Time zone</label><input id="timezone" name="statedTimeZone" value={timeZone ?? ""} readOnly required aria-describedby="timezone-help" /><small id="timezone-help">{timeZone ? "Detected from your browser so it matches the device times above." : "Your browser has not reported a valid time zone."}</small></div>
      ...
      <div className="form-actions"><button className="button" type="submit" disabled={status.kind === "busy" || !timeZone}>Continue on GitHub</button><p className={`form-status ${status.kind === "error" ? "error" : ""}`} role="status">{status.message}</p></div>
    </form>
  );
}
