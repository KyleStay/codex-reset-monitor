    ...
    <>
      <section className="page-hero">
        <div className="shell">
          ...
          <p className="lede">The monitor is an unofficial experiment built from observed events and approved public context. An optional local observer can read only privacy-minimized rate-limit metadata; the public site cannot see your Codex account or OpenAI’s internal systems.</p>
        </div>
      </section>
      <section className="section">
        <div className="shell content-grid">
          ...
          <div className="prose">
            ...
            <section id="labels"><p className="section-label">Evidence</p><h2>Labels are not interchangeable</h2><ul><li><b>Confirmed observation:</b> reviewed evidence of availability returning after a limit, or a deterministic local transition where usage drops while the official reset timestamp advances.</li><li><b>Inferred event:</b> plausible timing reconstructed from incomplete evidence.</li><li><b>Forecast:</b> an estimate made before the outcome.</li><li><b>Incident:</b> an official service-status record, not a personal reset.</li><li><b>Public signal:</b> an approved post or announcement.</li><li><b>Correction:</b> an append-only change that preserves the original record.</li></ul></section>
            <section id="privacy"><p className="section-label">Privacy</p><h2>Event timing, not account contents</h2><p>We never ask for Codex prompts, responses, source code, API credentials, cookies, session tokens, private repository data, screenshots, logs, or other account contents. Manual reports contain UTC event times, the stated time zone, surface, optional generalized plan tier, detection method, confidence, and optional short timing notes.</p><p>The opt-in local observer calls only Codex’s read-only rate-limit method. It stores sample time, primary-window usage percentage, reset timestamp, duration, exhaustion state, generalized plan tier, and deduplication state. It does not access conversations, task history, account identifiers, or device locale.</p><p>Reports are public GitHub issues and are associated with the submitter’s GitHub account under GitHub’s privacy terms. The published dataset deliberately omits author identity. GitHub provides authenticated submission and platform abuse controls; maintainers provide verification labels. Git history preserves corrections.</p></section>
