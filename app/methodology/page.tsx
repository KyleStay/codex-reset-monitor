    ...
    <>
      <section className="page-hero">
        <div className="shell">
          ...
          <p className="lede">The monitor is an unofficial experiment built from automatically observed full-reset transitions and source-backed historical inference. The local bridge reads only privacy-minimized quota and OpenAI-issued reset-credit metadata; the public site cannot see your Codex account or OpenAI’s internal systems.</p>
        </div>
      </section>
      <section className="section">
        <div className="shell content-grid">
          ...
          <div className="prose">
            ...
            <section id="labels"><p className="section-label">Evidence</p><h2>Labels are not interchangeable</h2><ul><li><b>Confirmed full reset:</b> reviewed evidence of availability returning after exhaustion, or a deterministic local transition to near-zero usage with an advanced official reset timestamp.</li><li><b>Queued reset:</b> OpenAI-issued reset-credit capacity that is available but not yet consumed.</li><li><b>Inferred event:</b> plausible timing reconstructed from incomplete evidence.</li><li><b>Forecast:</b> an estimate made before the outcome.</li><li><b>Incident:</b> an official service-status record, not a personal reset.</li><li><b>Public signal:</b> an approved post or announcement.</li><li><b>Correction:</b> an append-only change that preserves the original record.</li></ul></section>
            <section id="privacy"><p className="section-label">Privacy</p><h2>Reset metadata, not account contents</h2><p>We never ask for Codex prompts, responses, source code, API credentials, cookies, session tokens, private repository data, screenshots, logs, or other account contents. Optional manual reports contain UTC event times, the stated time zone, surface, optional generalized plan tier, detection method, confidence, and optional short timing notes.</p><p>The local telemetry bridge calls only Codex’s read-only <code>account/rateLimits/read</code> method. It retains up to 90 days of quota samples, the latest multi-bucket quota view, safe queued reset-credit metadata, full-reset detections, and deduplication state. It does not access aggregate usage, conversations, task or thread history, account identifiers, credentials, logs, or device locale.</p><p>Optional reports and automatic reset records are public GitHub issues. The published dataset deliberately omits author identity. Git history preserves corrections.</p></section>
            <section id="limitations"><p className="section-label">Limitations</p><h2>Not a promise of access</h2><p>The bridge cannot reconstruct private quota states from before it began sampling. Earlier events are best-effort public-source inferences and are labeled with their precision and evidence grade. Resets may also differ by product surface, plan, geography, account, policy, and unobserved OpenAI behavior. Official incidents can correlate with failures without causing usage-limit resets. With fewer than two verified resets, the published prior is not evidence of predictive skill.</p><div className="notice"><b>Forecasts are experimental estimates, never guarantees.</b> Do not plan critical work around them.</div></section>
            <section id="sources"><p className="section-label">Sources</p><h2>Compliant by design</h2><p>The automated official adapter reads OpenAI’s public status JSON and stores provenance, publication/retrieval times, canonical URL, minimal excerpt, normalized features, and a deduplication hash. Reset observations are collected from GitHub issues only after a repository maintainer adds the protected verification label. Public posts require the separate approval label. No “Tibo” account is guessed and no social site is scraped.</p><p>Historical research is maintained separately. Public reports and OpenAI repository comments may support an explicitly inferred event, cause assessment, contradiction, or future detection signal, but they never become confirmed training labels without a verified observation.</p></section>
